...
class GradientBackground {
  ...
  constructor(sceneManager: any) {
    ...
    // AWS Club theme: vibrant violet/fuchsia/rose blobs over deep purple base
    this.uniforms = {
      uTime: { value: 0 },
      uResolution: { value: new THREE.Vector2(window.innerWidth, window.innerHeight) },
      // Bright violet blob
      uColor1: { value: new THREE.Vector3(0.545, 0.361, 0.965) },   // violet-500 (#8B5CF6)
      // Deep indigo blob (visible, not black)
      uColor2: { value: new THREE.Vector3(0.180, 0.120, 0.420) },   // deep indigo
      // Bright fuchsia blob
      uColor3: { value: new THREE.Vector3(0.851, 0.275, 0.937) },   // fuchsia-500 (#D946EF)
      // Medium dark purple blob
      uColor4: { value: new THREE.Vector3(0.220, 0.060, 0.340) },   // dark purple
      // Rose / warm accent blob
      uColor5: { value: new THREE.Vector3(1.000, 0.400, 0.200) },   // AWS orange (#FF9900-ish warm)
      // Deep violet blob
      uColor6: { value: new THREE.Vector3(0.280, 0.100, 0.500) },   // deep violet
      uSpeed: { value: 1.2 }, uIntensity: { value: 1.1 },
      uTouchTexture: { value: null }, uGrainIntensity: { value: 0.12 },
      uDarkNavy: { value: new THREE.Vector3(0.039, 0.020, 0.090) }, // visible deep purple base
      uGradientSize: { value: 0.45 }, uGradientCount: { value: 12.0 },
      uColor1Weight: { value: 0.5 }, uColor2Weight: { value: 1.2 }
    };
  }
  init() {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        vec3 getGradientColor(vec2 uv, float time) {
          ...
          color = clamp(color, vec3(0.0), vec3(0.85)) * uIntensity;
          float lum = dot(color, vec3(0.299, 0.587, 0.114));
          color = mix(vec3(lum), color, 1.5);
          color = pow(color, vec3(1.1));
          float brightness = length(color);
          color = mix(uDarkNavy, color, max(brightness * 1.0, 0.15));
          return color;
        }
...
class GradientApp {
  ...
  constructor(container: HTMLElement) {
    ...
    this.scene.background = new THREE.Color(0x0a0517);
...
/**
 * Liquid Gradient Background — renders a fixed full-page interactive 
 * Three.js animated gradient. Use as a global background layer.
 */
export default function LiquidGradientBackground({ className = '', isPaused = false }: LiquidGradientBackgroundProps) {
  ...
  useEffect(() => {
    if (appRef.current) {
      appRef.current.gradientBackground.isPaused = isPaused;
    }
  }, [isPaused]);
...
export { LiquidGradientBackground };
