...
export default function Footer() {
  ...
  return (
    <footer className="relative mt-24 py-16 overflow-hidden bg-black/40 backdrop-blur-[60px] border-t border-white/[0.08] shadow-[inset_0_1px_0_0_rgba(255,255,255,0.2),0_-40px_100px_rgba(0,0,0,0.6)]">
      ...
      <div className="absolute -top-64 -right-40 w-[600px] h-[600px] bg-violet-600/20 rounded-[100%] blur-[160px] pointer-events-none mix-blend-screen animate-pulse" style={{ animationDelay: '2s' }}></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 mb-16">
          ...
          <div className="lg:col-span-1" style={{ perspective: '1000px' }}>
            ...
            <p className="text-base text-white/70 mb-8 leading-relaxed font-light drop-shadow-sm">
              Empowering students through hands-on cloud computing experience, networking, and real-world project builds.
            </p>
              ...
              {[
                {
                  ...
                  svg: <path fillRule="evenodd" d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" clipRule="evenodd" />
                },
                {
                  name: "Instagram",
                ...
                <a key={idx} href={item.link} target="_blank" rel="noopener noreferrer" 
                  ...
                  <svg className="h-5 w-5 drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)] group-hover:drop-shadow-[0_4px_8px_rgba(255,100,255,0.5)] transition-all duration-300" fill="currentColor" viewBox={item.viewBox || "0 0 24 24"} aria-hidden="true">
                    {item.svg}
                  </svg>
                </a>
              ))}
          ...
          <div className="flex flex-col gap-4">
            <h3 className="text-white/50 font-semibold tracking-widest uppercase text-xs drop-shadow-md">Quick Links</h3>
            <ul className="space-y-4">
              {['About Us', 'Upcoming Events', 'Gallery', 'FAQ'].map((link, idx) => (
                <li key={idx}>
                  <a href="#" className="flex items-center group">
                    <span className="text-white/70 group-hover:text-white group-hover:drop-shadow-[0_0_8px_rgba(255,255,255,0.6)] transition-all duration-300 font-medium">{link}</span>
                    <span className="ml-2 opacity-0 group-hover:opacity-100 group-hover:translate-x-1.5 transition-all duration-300 text-fuchsia-400 drop-shadow-[0_0_8px_rgba(255,100,255,0.6)] border-white/10 font-black">→</span>
                  </a>
                </li>
              ))}
            </ul>
          ...
          {/* Resources */}
          <div className="flex flex-col gap-4">
            <h3 className="text-white/50 font-semibold tracking-widest uppercase text-xs drop-shadow-md">Resources</h3>
            <ul className="space-y-4">
              {[
                { label: 'AWS Certification Guide', link: '#' },
                { label: 'Project Walkthroughs', link: '#' },
                { label: 'Tech Blog', link: '#' },
                { label: 'AWS Free Tier', link: 'https://aws.amazon.com/free/', external: true }
              ].map((item, idx) => (
                <li key={idx}>
                  <a href={item.link} target={item.external ? "_blank" : "_self"} rel={item.external ? "noopener noreferrer" : ""} 
                    ...
                    {item.label}
                    {item.external && <span className="ml-1.5 text-[10px] text-fuchsia-400 opacity-60 group-hover:opacity-100 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all drop-shadow-[0_0_5px_rgba(255,100,255,0.5)]">↗</span>}
                  </a>
                </li>
              ))}
            </ul>
          ...
          <div className="flex flex-col gap-4" style={{ perspective: '800px' }}>
            <h3 className="text-white/50 font-semibold tracking-widest uppercase text-xs drop-shadow-md">Stay Updated</h3>
            <p className="text-white/70 text-base mb-2 font-light leading-relaxed drop-shadow-sm">Get notified about our upcoming workshops and exclusive networking events.</p>
        ...
        <div className="pt-8 flex flex-col md:flex-row justify-between items-center gap-6 mt-8 relative">
          ...
          <div className="flex space-x-6">
            {['Code of Conduct', 'Privacy Policy', 'Terms'].map((item, idx) => (
              <a key={idx} href="#" className="relative group text-sm text-white/50 hover:text-white transition-all duration-300 font-semibold drop-shadow-md">
                {item}
